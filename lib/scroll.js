...
$(document).ready(function () {
  ...
  var arrowDeltas$ = keyDowns$.filter(function (key) {
    return key.which === 40 || key.which === 38;
  }).map(function (key) {
    return key.which === 40 ? 1 : -1;
  });

  var mobileButtonDeltas$ = mobileButtons$.map(function (e) {
    return e.target.innerText === "Next" ? 1 : -1;
  });

  var wheelDeltas$ = wheels$.filter(function (wheel) {
  ...
  var scrollActions = Rx.Observable.merge(arrowDeltas$, wheelDeltas$, mobileButtonDeltas$).map(function (delta) {
    return function (currentScreen) {
      return nextScreen(currentScreen + delta);
    };
  });

  var clickActions = menuClicks$.filter(function (click) {
